...
import com.otula.uiutils.DialogUtils.DialogListener.ConfirmationStatus;
...
public final class DialogUtils {
	...
	public static void showConfirmationDialog(final Activity activity, final int resId, final DialogListener listener, boolean showCancel, final int tag){
		...
		builder.setPositiveButton(R.string.button_yes, new DialogInterface.OnClickListener() {		
			...
			public void onClick(DialogInterface dialog, int which) {
				dialog.dismiss();
				listener.dialogClosed(ConfirmationStatus.ACCEPTED, tag);
			}
		});
		builder.setNegativeButton(R.string.button_no, new DialogInterface.OnClickListener() {		
			...
			public void onClick(DialogInterface dialog, int which) {
				dialog.dismiss();
				listener.dialogClosed(ConfirmationStatus.REJECTED, tag);
			}
		});
		if(showCancel){
			builder.setNeutralButton(R.string.button_cancel, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					dialog.dismiss();
					listener.dialogClosed(ConfirmationStatus.CANCELLED, tag);
				}
			});
		}
	...
	public static <T> void showSelectionDialog(final Activity activity, final Map<T,String> values, final DialogListener listener, T selectedItem, final int tag){
		if(values == null || values.isEmpty()){
			LogUtils.warn(CLASS_NAME, "showSelectionDialog", "Empty value list given for selection.");
			listener.selectionClosed(null, null, tag);
			return;
		}
		
		AlertDialog.Builder sel = new AlertDialog.Builder(activity);
		sel.setTitle(R.string.title_dialog_selection);
		
		final LinkedList<T> idList = new LinkedList<T>();
